...
if($_POST) {
  ...
  if(isset($_POST['vorname'])) {
    ...
    $email_body .= "<div><label><b>Vorname:</b></label>&nbsp;<span>".$vorname."</span></div>";
  }

  if(isset($_POST['nachname'])) {
    $nachname = filter_var($_POST['nachname'], FILTER_SANITIZE_STRING);
    $email_body .= "<div><label><b>Nachname:</b></label>&nbsp;<span>".$nachname."</span></div>";
  }

  if(isset($_POST['email'])) {
    $email = str_replace(array("\r", "\n", "%0a", "%0d"), '', $_POST['email']);
    $email = filter_var($email, FILTER_VALIDATE_EMAIL);
    $email_body .= "<div><label><b>Email-Adresse:</b></label>&nbsp;<span>".$email."</span></div>";
  }
